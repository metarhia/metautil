...
const generateUUID = () => {
  const data = [
    randomPrefetcher.next(),
    randomPrefetcher.next(),
    randomPrefetcher.next(),
    randomPrefetcher.next(),
  ];
  data[2][0] = (data[2][0] & 0x0f) | 0x40;
  data[2][2] = (data[2][2] & 0x3f) | 0x80;
  const [h1, h2, h3, h4] = data.map((buf) => buf.toString('hex'));
