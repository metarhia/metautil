...
const unflatObject = (source, fieldNames) => {
  const result = {};
  const tempObj = { ...source };
  for (const prefix of fieldNames) {
    if (tempObj[prefix] !== undefined) {
      throw new Error(`Can not combine keys: key "${prefix}" already exists`);
    }
    for (const [key, value] of Object.entries(tempObj)) {
      if (key.startsWith(prefix)) {
        const newKey = key.slice(prefix.length).toLowerCase();
        result[prefix] = { ...result[prefix], [newKey]: value };
        delete tempObj[key];
        continue;
      }
      if (isObject(value)) {
        for (const [nestedKey, nestedValue] of Object.entries(value)) {
          ...
          if (tempObj[newKey] !== undefined) {
            throw new Error(
              `Can not combine keys: key "${newKey}" already exists`,
            );
          }
