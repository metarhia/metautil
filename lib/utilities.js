...
const parseEvery = (s = '') => {
  ...
  return { MM, DD, wd, hh, mm, ms: ms > 0 ? ms * 1000 : -1 };
};
...
const nextEvent = (every, date = new Date()) => {
  ...
  if (every.wd > -1 && every.wd !== wd) return -1;
  const hh = date.getUTCHours();
  if (every.hh > -1) {
    ms += (every.hh - hh) * DURATION_UNITS.h;
  }
  const mm = date.getUTCMinutes();
  if (every.mm > -1) {
    ms += (every.mm - mm) * DURATION_UNITS.m;
  }
  ms *= 1000;
...
const flatObject = (sourceObj, nameFields) => {
  const targetObj = {};
  ...
  for (const [key, value] of Object.entries(sourceObj)) {
    ...
    if (nameFields.length > 0 && !nameFields.includes(key)) {
      targetObj[key] = { ...value };
      continue;
    }

    for (const [key2, value2] of Object.entries(value)) {
      const newKey = `${key}${toUpperCamel(key2)}`;

      if (sourceObj[newKey] !== undefined)
        throw new Error('flatObject: duplicate key');

      targetObj[newKey] = value2;
    }
  }
