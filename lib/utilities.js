...
const deflatObject = (source, fieldName) => {
  const result = {};
  for (const [key, value] of Object.entries(source)) {
    if (typeof value === 'string') {
      for (let i = 0; i < key.length; i++) {
        if (inRange(key[i], 'A', 'Z')) {
          const firstWord = key.slice(0, i);
          const secondWord = key.slice(i).toLowerCase();
          if (fieldName && fieldName !== firstWord) {
            result[key] = value;
            break;
          }
          if (source[firstWord] !== undefined) {
            throw new Error(`${firstWord} key already exist`);
          }
          result[firstWord] = { ...result[firstWord], [secondWord]: value };
        }
      }
      continue;
    }
    if (fieldName && typeof value === 'object' && !Array.isArray(value)) {
      for (const [nestedKey, nestedValue] of Object.entries(value)) {
        const newKey = `${key}${toUpperCamel(nestedKey)}`;
        if (source[newKey] !== undefined) {
          throw new Error(`${newKey} key already exist`);
        }
