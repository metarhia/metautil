...
const flatObject = (obj, options = {}) => {
  ...
  for (const [key, value] of entries) {
    ...
    for (const [nestedKey, nestedValue] of nestedEntries) {
      ...
      const isKeyAlreadyExists = result[newKey] !== undefined;
      if (isKeyAlreadyExists) throw new Error(`Key ${newKey} already exists`);

      result[newKey] = isObject(nestedValue) ? { ...nestedValue } : nestedValue;
    }
  }
