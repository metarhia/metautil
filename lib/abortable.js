...
const STATES = ['IDLE', 'RUNNING', 'ABORTED'];
...
class Abortable extends EventEmitter {
  ...
  static {
    for (const state of STATES) {
      Object.defineProperty(Abortable, state, {
        enumerable: true,
  ...
  constructor(delay = 0, throwOnAborted = false) {
    super();
    this.delay = delay;
  ...
  async run(task, ...args) {
    if (this.#state !== Abortable.IDLE)
      return Promise.reject(
        new Error('Cannot run the task: task is already running or aborted'),
      );
    this.emit('running');
    this.#state = Abortable.RUNNING;
    try {
  ...
  resetTimeout(delay) {
    if (this.#state !== Abortable.RUNNING) return;
    const { signal: timeoutSignal } = this.#timeoutAc;
    const { signal: taskSignal } = this.#taskAc;
    if (timeoutSignal.aborted || taskSignal.aborted) return;
