...
const makePrivate = (instance) => {
  ...
  for (const fieldName of fields) {
    ...
    if (utilities.isConstant(fieldName)) {
...
const flatObject = (sourceObject, fieldNames = []) => {
  ...
  for (const [key, value] of Object.entries(sourceObject)) {
    if (!utilities.isHashObject(value)) {
      target[key] = value;
      continue;
    }
    ...
    for (const [childKey, childValue] of Object.entries(value)) {
      const combined = `${key}${utilities.toUpperCamel(childKey)}`;
...
const getSignature = (method) => {
  ...
  const signature = utilities.between(src, '({', '})');
  if (signature === '') return [];
