...
class StreamReadable extends Readable {
  ...
  constructor(eventIterable, options = {}) {
    super({ objectMode: true, ...options });
    this.#iterator = eventIterable[Symbol.asyncIterator]();
    this.#reading = true;
    this.#pull();
  }
  ...
  toEventEmitter(eventName = 'data') {
    const emitter = new Emitter();
    this.on('data', (chunk) => {
      emitter.emit(eventName, chunk);
    });
    this.on('error', (error) => {
...
class Stream {
  static from(iterable, options) {
    return new StreamReadable(iterable, options);
  }
}

module.exports = { Emitter, Stream };
