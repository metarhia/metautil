...
class Emitter {
  ...
  emit(eventName, value) {
    ...
    const handlers = listeners.slice().concat(onceListeners);
  ...
  off(eventName, listener) {
    ...
    if (onceIndex > -1) listeners.splice(onceIndex, 1);
  }
  ...
  listeners(eventName) {
    if (eventName) {
      ...
      return events.slice().concat(onceEvents);
    }
  ...
  eventNames() {
    ...
    const onceNames = this.#onceEvents.keys();
    const allNames = names.concat(onceNames);
