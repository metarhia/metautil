...
class List {
  ...
  #findPosition(index) {
    ...
    if (actualIndex === 0) {
      this.#posResult.node = this.#head;
      this.#posResult.offset = this.#head.readIndex;
      this.#cachedNode = this.#head;
      this.#cachedNodeStart = 0;
      return this.#posResult;
    }

    if (actualIndex === this.#length - 1) {
      this.#posResult.node = this.#tail;
      this.#posResult.offset = this.#tail.writeIndex - 1;
      this.#cachedNode = this.#tail;
      this.#cachedNodeStart = this.#length - this.#tail.length;
      return this.#posResult;
    }

    const hasMiddle = this.#head.next && this.#head.next !== this.#tail;
    if (this.#uniformMiddle && hasMiddle) {
      ...
      if (inMiddle && beforeTail) {
        ...
        if (this.#skipPointers.length > 0) {
          ...
          if (skipIndex < this.#skipPointers.length) {
            ...
            this.#posResult.offset = node.readIndex + offsetInNode;
            this.#cachedNode = node;
            const base = headLength + (nodeNumber - 1) * this.#nodeSize;
            this.#cachedNodeStart = base;
            return this.#posResult;
          }
        }
      }
    ...
    if (
      this.#cachedNode &&
      actualIndex >= this.#cachedNodeStart &&
      actualIndex < this.#cachedNodeStart + this.#cachedNode.length
    ) {
      this.#posResult.node = this.#cachedNode;
      this.#posResult.offset =
        this.#cachedNode.readIndex + (actualIndex - this.#cachedNodeStart);
      return this.#posResult;
    }
    ...
    if (actualIndex < cumulativeIndex) {
      while (node) {
        ...
        if (actualIndex >= cumulativeIndex && actualIndex < end) {
          this.#posResult.node = node;
          const off = node.readIndex + (actualIndex - cumulativeIndex);
          this.#posResult.offset = off;
          this.#cachedNode = node;
