...
class List {
  ...
  groupBy(getKey) {
    if (typeof getKey !== 'function') throw new TypeError('getKey must be a function');
  ...
  sort(compare) {
    if (compare !== undefined && typeof compare !== 'function') throw new TypeError('compare must be a function');
  ...
  sum(fn) {
    if (fn !== undefined && typeof fn !== 'function') throw new TypeError('fn must be a function');
    let total = 0;
    let node = this.#head;
    if (fn) {
      while (node !== null) {
