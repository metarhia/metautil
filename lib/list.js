...
class List {
  ...
  rotate(n = 1) {
    const size = this.#size;
    if (size < 2 || !Number.isInteger(n)) return;
  ...
  move(from, to) {
    ...
    node.next = null;
    this.#size--;
    const before = t < this.#size ? this.#nodeAt(t) : null;
  ...
  splitAt(index) {
    ...
    if (at === 0 || at === size) {
      ...
      if (at === size) return { before: copy, after: empty };
    }
